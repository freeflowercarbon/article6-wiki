# 🌍 ITMO Transfer Tracker | ITMO移転トラッカー

=== "🇬🇧 English"

    > Last updated: **2026-07-26** — Single source of truth for **confirmed Article 6.2 ITMO transfers** worldwide. Status anywhere else in this wiki should match this page; if it doesn't, this page wins.

    ## Confirmed A6.2 ITMO Transfers (all known, chronological)

    As of June 2026, **106 bilateral arrangements across 53 host countries** have been formalized, but only the following international transfers have actually been completed.[^stat]

    | # | Host → Acquirer | Transfer Date | Volume | Project | Significance |
    |---|----------------|---------------|--------|---------|--------------|
    | 1 | Thailand 🇹🇭 → Switzerland 🇨🇭 | **December 2023** | 1,916 units | Bangkok E-Bus Programme (diesel→electric buses; KliK Foundation) | 🏆 **World's first-ever completed A6.2 ITMO transfer**[^th1] |
    | 2 | Norway 🇳🇴 → Switzerland 🇨🇭 | **June 17, 2025** | Undisclosed | CDR (carbon removal; structured by ClimeFi) | 🏆 **First-ever CDR-category ITMO transfer**[^no1] |
    | 3 | Ghana 🇬🇭 → Switzerland 🇨🇭 | **July 7, 2025** | 11,733 units | Transformative Cookstove Activity in Rural Ghana (ACT Group / KliK) | 🏆 **Africa's first A6.2 ITMO issued for NDC use**[^gh1] |
    | 4 | Thailand 🇹🇭 → Japan 🇯🇵 | **November 11, 2025** | 1,009 tCO₂e (vintage 2021) | TH014 — 5MW Floating Solar on Industrial Reservoir (acquired by MOEJ) | 🏆 **First-ever ITMO transfer under the JCM**[^th2] |
    | 5 | Maldives 🇲🇻 → Japan 🇯🇵 | **December 17, 2025** | 433 tCO₂e (vintage 2021–22) | MV002 — Smart Mini Grid System at Addu City (acquired by MOEJ) | 2nd JCM ITMO transfer[^mv1] |
    | 6 | Thailand 🇹🇭 → Switzerland 🇨🇭 | **April 2, 2026** | 49,717 units (vintage 2023–24) | Bangkok E-Bus Programme (2nd issuance) | Largest single A6.2 issuance to date[^th3] |
    | 7 | Switzerland 🇨🇭 → Sweden 🇸🇪 | **May 6, 2026** | Undisclosed (pilot) | CDR pilot (two Swedish buyers received negative-emission credits; Switzerland acting as CDR host) | 🏆 **First A6.2 transfer where Switzerland acts as host; Sweden's first A6.2 transfer**[^swse1] |
    | 8 | Mongolia 🇲🇳 → Japan 🇯🇵 | **July 6, 2026** | 86,564 tCO₂e | MN003, MN004, MN006, MN007 — solar + battery-storage projects (JC decision Jun 30, 2026; acquired by MOEJ) | 🏆 **Mongolia's first-ever JCM ITMO transfer**; Japan's 3rd–4th JCM transfer[^mn1] |

    ### Related milestone (Article 6.4 / PACM — separate mechanism)

    | Date | Event |
    |------|-------|
    | **February 26, 2026** | **First-ever A6.4ERs issued under PACM**: Myanmar efficient cookstoves, 58,428 tCO₂e (South Korea acquiring) — see [PACM](concepts/pacm.md) |

    ## Authorized — but NOT yet transferred

    Common error in market commentary (and previously in this wiki): treating *agreements*, *authorizations*, or *purchase contracts* as transfers. The following are **not** completed transfers:

    | Host | Partner | Actual status | First transfer est. |
    |------|---------|---------------|---------------------|
    | Peru 🇵🇪 | Switzerland 🇨🇭 | World's **first bilateral agreement** (Oct 2020) + world's first commercial ITMO **purchase agreement** (Microsol, Nov 2021); Tuki Wasi cookstoves **authorized Mar 20, 2026**; RENAMI registered Feb 2026 | 2026–27 |
    | Georgia 🇬🇪 | Switzerland 🇨🇭 | Bilateral 2022; public-building energy-efficiency retrofits under implementation | 2026–27 |
    | Laos 🇱🇦 | Japan 🇯🇵 (JCM) | A6.2 authorization (enteric methane) reported — transfer unconfirmed（要確認） | 2026–27 |
    | Malawi 🇲🇼 | Switzerland 🇨🇭 | AIR Aug 2025; Dairy Biogas Programme authorized Apr 6, 2026 (KliK) | 2027+ |
    | Vanuatu 🇻🇺 | Switzerland 🇨🇭 | Project launched (solar electrification, UNDP) — no issuance | 2027+ |
    | Ghana 🇬🇭 | Singapore 🇸🇬 | RFP1 contract Sep 2025 (Kwahu Landscape Restoration ARR ~1.35Mt) | 2026–27 |
    | Peru/Paraguay 🇵🇪🇵🇾 | Singapore 🇸🇬 | RFP1 contracts Sep 2025 (REDD+/grassland) | 2026–27 |
    | Cambodia 🇰🇭 | Japan 🇯🇵 (JCM) | JC credit-issuance decision (Mar 2026) — transfer not yet announced | 2026 |

    ## Scoreboard by acquiring country

    | Acquirer | Completed transfers | Volume acquired | Notes |
    |----------|--------------------:|----------------:|-------|
    | Switzerland 🇨🇭 | **4** (TH ×2, NO, GH) | ~63,000+ units | World leader (acquirer); also acted as CDR *host* to Sweden (May 2026); legally mandated demand (CO2 Act) |
    | Japan 🇯🇵 | **3** (TH, MV, MN) | 88,006 t | First JCM ITMOs Nov–Dec 2025; Mongolia's first JCM transfer Jul 2026 (86,564t across 4 projects); 32 JCM partners; more JC issuance decisions queued |
    | Singapore 🇸🇬 | 0 | — | **11 IAs signed** (Philippines 11th, Apr 2026; Cambodia MOU only); RFP1 contracted; first delivery 2026–27 |
    | **Sweden 🇸🇪** | **1** (CH CDR pilot) | Undisclosed | **First Swedish A6.2 transfer May 6, 2026** — CDR credits from Switzerland; multiple bilateral agreements as acquirer |
    | Norway 🇳🇴 | 0 (as acquirer) | — | 7 NOGER bilaterals; was *host* in the CDR transfer to Switzerland (Jun 2025) |
    | South Korea 🇰🇷 | 0 under A6.2 | — | Acquiring 58,428t A6.4ERs (Myanmar PACM, Feb 2026); India bilateral Apr 2026 |

    ## Why so few transfers?

    1. **First monitoring cycle**: typically 18–24 months after IA signature before first verified vintage.
    2. **Authorization ≠ transfer**: host authorization, registry entry, CA application, and registry-to-registry transfer are separate steps.
    3. **Infrastructure**: host registries (e.g., Peru RENAMI Feb 2026, Indonesia SRUK trials Apr 2026, Vietnam decree May 2026) only now coming online.
    4. **Buyer caution**: Switzerland's BeZero risk reviews; Singapore's "road test" approach.

    [^stat]: A6 Implementation Partnership, implementation status (as of March 2026: 106 bilateral arrangements, 53 host countries): <https://a6partnership.org/a6-implementation-status/current-status-of-article-6-2-implementation>
    [^th1]: KliK Foundation, "First ever ITMOs for NDC use" (Bangkok E-Bus, Dec 2023): <https://www.klik.ch/en/news/news-article/first-ever-itmos-for-ndc-use>
    [^no1]: ClimeFi, first Norway–Switzerland CDR ITMO transfer (Jun 17, 2025): <https://www.climefi.com/blog-posts/climefi-structures-itmo-transfer-between-norway-and-switzerland-under-article-6-of-the-paris-agreement>
    [^gh1]: KliK Foundation, Ghana first African ITMO issuance (Jul 7, 2025, 11,733 units): <https://www.klik.ch/en/news/news-article/first-itmo-transfer-switzerland-ghana>
    [^th2]: GEC/JCMA, "Completion of the first international transfer of mitigation outcomes for JCM credits (Thailand)" — TH014, 1,009 tCO₂e, transfer Nov 11, 2025, JC7 decision Oct 30, 2025, credit auth. no. JCM-TH-JP-2018-3026-01401: <https://gec.jp/jcm/agency/en/news/information-611/>
    [^mv1]: JCM Maldives ITMO transfer (MV002, 433 tCO₂e, Dec 17, 2025; JC5 decision Dec 16, 2025): <https://www.jcm.go.jp/jc/information/20251217/>; QC Intel: <https://www.qcintel.com/carbon/article/japan-maldives-complete-itmo-transfer-under-jcm-55347.html>
    [^th3]: Energy Absolute, Bangkok E-Bus 2nd ITMO issuance (49,717 units, Apr 2, 2026): <https://www.energyabsolute.co.th/en/newsroom/news/490/>
    [^swse1]: Switzerland–Sweden CDR pilot ITMO transfer (May 6, 2026): two Swedish buyers received negative-emission ITMOs in a pilot transaction; Switzerland acted as CDR host country. Source: crawl_A 2026-06-21 (Carbon Pulse / ClimeFi secondary reports)
    [^mn1]: GEC/JCMA, Japan–Mongolia first JCM ITMO transfer (MN003, MN004, MN006, MN007; 86,564 tCO₂e; JC decision Jun 30, 2026; transfer completed Jul 6, 2026): <https://gec.jp/jcm/agency/en/news/information-20260706/>

=== "🇯🇵 日本語"

    > 最終更新: **2026-07-26** — 世界の**確認済みA6.2 ITMO移転**の一元管理ページ。本Wikiの他ページの記載と食い違う場合は本ページが正。

    ## 確認済みA6.2 ITMO移転（全件・時系列）

    2026年6月時点で**106件の二国間協定（53ホスト国）**が形式化されているが、実際に完了した国際移転は以下のみ。[^stat]

    | # | ホスト → 調達国 | 移転日 | 量 | プロジェクト | 意義 |
    |---|---------------|--------|---|------------|------|
    | 1 | タイ 🇹🇭 → スイス 🇨🇭 | **2023年12月** | 1,916ユニット | バンコクE-Busプログラム（ディーゼル→EVバス；KliK財団） | 🏆 **世界初のA6.2 ITMO移転**[^th1] |
    | 2 | ノルウェー 🇳🇴 → スイス 🇨🇭 | **2025年6月17日** | 非公開 | CDR（炭素除去；ClimeFi組成） | 🏆 **初のCDRカテゴリITMO移転**[^no1] |
    | 3 | ガーナ 🇬🇭 → スイス 🇨🇭 | **2025年7月7日** | 11,733ユニット | ガーナ農村部改良かまど（ACT Group／KliK） | 🏆 **アフリカ初のNDC用途A6.2 ITMO発行**[^gh1] |
    | 4 | タイ 🇹🇭 → 日本 🇯🇵 | **2025年11月11日** | 1,009 tCO₂e（2021ビンテージ） | TH014 — 産業用貯水池5MW浮体式太陽光（環境省取得） | 🏆 **JCM史上初のITMO移転**[^th2] |
    | 5 | モルディブ 🇲🇻 → 日本 🇯🇵 | **2025年12月17日** | 433 tCO₂e（2021〜22ビンテージ） | MV002 — アッドゥ市スマートミニグリッド（環境省取得） | JCM2件目のITMO移転[^mv1] |
    | 6 | タイ 🇹🇭 → スイス 🇨🇭 | **2026年4月2日** | 49,717ユニット（2023〜24ビンテージ） | バンコクE-Bus（第2回発行） | A6.2単一発行として過去最大[^th3] |
    | 7 | スイス 🇨🇭 → スウェーデン 🇸🇪 | **2026年5月6日** | 非公開（パイロット） | CDRパイロット（スウェーデンの2バイヤーが負の排出クレジットを受領；スイスがCDRホスト役） | 🏆 **スイスがホスト役として初のA6.2移転；スウェーデン初のA6.2取得**[^swse1] |
    | 8 | モンゴル 🇲🇳 → 日本 🇯🇵 | **2026年7月6日** | 86,564 tCO₂e | MN003・MN004・MN006・MN007 — 太陽光＋蓄電池プロジェクト（JC決定2026年6月30日；環境省取得） | 🏆 **モンゴル初のJCM ITMO移転**；日本にとって3〜4件目のJCM移転[^mn1] |

    ### 関連マイルストーン（Article 6.4 / PACM — 別メカニズム）

    | 日付 | イベント |
    |------|---------|
    | **2026年2月26日** | **PACM初のA6.4ER発行**：ミャンマー改良かまど、58,428 tCO₂e（韓国が取得） — [PACM](concepts/pacm.md)参照 |

    ## 授権済み・未移転（移転と混同されやすい案件）

    市場解説（および本Wikiの旧記載）でよくある誤りは、*協定*・*授権*・*購入契約*を移転と混同すること。以下は移転**未完了**：

    | ホスト | パートナー | 実際のステータス | 初回移転見込み |
    |--------|----------|----------------|--------------|
    | ペルー 🇵🇪 | スイス 🇨🇭 | **世界初の二国間協定**（2020年10月）＋世界初の商業ITMO**購入契約**（Microsol、2021年11月）；Tuki Wasi**授権 2026年3月20日**；RENAMI登録 2026年2月 | 2026〜27年 |
    | ジョージア 🇬🇪 | スイス 🇨🇭 | 二国間協定2022年；公共建築物省エネ改修が実施中 | 2026〜27年 |
    | ラオス 🇱🇦 | 日本 🇯🇵（JCM） | A6.2授権（腸内メタン）報道あり — 移転未確認（要確認） | 2026〜27年 |
    | マラウィ 🇲🇼 | スイス 🇨🇭 | AIR 2025年8月；乳牛バイオガス授権 2026年4月6日（KliK） | 2027年〜 |
    | バヌアツ 🇻🇺 | スイス 🇨🇭 | プロジェクト開始（太陽光電化、UNDP） — 発行なし | 2027年〜 |
    | ガーナ 🇬🇭 | シンガポール 🇸🇬 | RFP1契約 2025年9月（Kwahu景観再生ARR 約135万t） | 2026〜27年 |
    | ペルー／パラグアイ 🇵🇪🇵🇾 | シンガポール 🇸🇬 | RFP1契約 2025年9月（REDD+／草地） | 2026〜27年 |
    | カンボジア 🇰🇭 | 日本 🇯🇵（JCM） | JC発行決定（2026年3月） — 移転発表はまだ | 2026年 |

    ## 調達国別スコアボード

    | 調達国 | 完了移転数 | 取得量 | 備考 |
    |--------|----------:|-------:|------|
    | スイス 🇨🇭 | **4件**（タイ×2、ノルウェー、ガーナ） | 約63,000+ユニット | 世界首位（調達側）；2026年5月にCDRホストとしてスウェーデンへ移転実績も；CO2法による法的調達義務 |
    | 日本 🇯🇵 | **3件**（タイ、モルディブ、モンゴル） | 88,006t | 2025年末にJCM初ITMO実現；2026年7月モンゴル初移転（4件86,564t）；JCM32カ国；JC発行決定が後続 |
    | シンガポール 🇸🇬 | 0件 | — | **IA11件署名済み**（フィリピン第11号・2026年4月；カンボジアはMOUのみ）；RFP1契約済み；初回納入2026〜27年 |
    | **スウェーデン 🇸🇪** | **1件**（スイスCDRパイロット） | 非公開 | **2026年5月6日スウェーデン初のA6.2取得** — スイスCDRクレジット；調達側として複数二国間協定を保有 |
    | ノルウェー 🇳🇴 | 0件（調達側として） | — | NOGER 7カ国；対スイスCDR移転（2025年6月）では*ホスト*側 |
    | 韓国 🇰🇷 | A6.2では0件 | — | A6.4ERを58,428t取得（ミャンマーPACM、2026年2月）；インド二国間 2026年4月 |

    ## なぜ移転がこれほど少ないのか

    1. **初回モニタリングサイクル**：IA署名から初回検証済みビンテージまで通常18〜24ヶ月。
    2. **授権≠移転**：ホスト国授権、登録簿記載、CA適用、登録簿間移転は別個のステップ。
    3. **インフラ**：ホスト国登録簿（ペルーRENAMI 2026年2月、インドネシアSRUK試行 2026年4月、ベトナム政令 2026年5月）が今ようやく稼働段階。
    4. **バイヤーの慎重姿勢**：スイスのBeZeroリスク審査；シンガポールの「ロードテスト」アプローチ。

    ## 関連ページ
    - [マトリクス](matrix.md) — 二国間合意の全体像
    - [ITMO](concepts/itmo.md) — 概念解説
    - [JCM](concepts/jcm.md) — 日本の二国間クレジット制度
    - [PACM](concepts/pacm.md) — Article 6.4メカニズム
