# Korea 🇰🇷 | 韓国 — Article 6.2 Acquiring Country

=== "🇬🇧 English"

    ## Overview

    Korea operates **Asia's largest cap-and-trade system** (K-ETS, since 2015) and is positioning as an Article 6.2 acquiring country. Korea's 2030 NDC (40% reduction vs. 2018) creates structural demand for international credits, particularly as Phase 4 (2026–2030) design includes expanded A6.2-compatible credit acceptance. Korea has signed an MOU with **Indonesia (June 2024)** on Article 6 carbon project cooperation, and secured a project-level authorization from Cambodia (July 2025). India-Korea Article 6.2 cooperation was formalized in April 2026.[^krndc]

    | Item | Detail |
    |------|--------|
    | NDC Target | 40% reduction vs. 2018 by 2030; net-zero 2050 |
    | Carbon System | K-ETS (Korea Emissions Trading Scheme, 2015–present) |
    | K-ETS Price (2024–25) | KRW 8,000–18,000/t (≈ US$6–13) |
    | K-ETS Price (2030 est.) | KRW 20,000–40,000/t (≈ US$15–29) |
    | International Credit Limit | 5% of allocation (K-ETS Phase 3) |
    | Key Ministry | Ministry of Environment (MOE); Ministry of Foreign Affairs |
    | A6.2 Role | Potential acquiring country (framework under development) |

    ## NDC Achievement Status

    | Metric | Assessment |
    |--------|-----------|
    | Current progress | 🟡 Moderately on track; heavy industry (steel, petrochemicals) remains difficult |
    | 2030 target (40%) | 🟡 Achievable but requires Phase 4 K-ETS price increase + international credits |
    | International credit use | K-ETS Phase 4 design includes A6.2-compatible credit pathway (under finalization) |
    | GST 2023 Implication | Korea expected to raise 2035 NDC; A6.2 credit pathway needs to be locked in pre-2026 |

    > Source: Korea NDC — <https://www4.unfccc.int/sites/NDCStaging/Pages/Party.aspx?party=KOR>

    ## Bilateral A6.2 Agreements

    | Host Country | Status | Date | Notes |
    |-------------|--------|------|-------|
    | Indonesia 🇮🇩 | 📋 MOU | Jun 2024 | Article 6 carbon project cooperation MOU; focus on industry projects[^krid] |
    | Cambodia 🇰🇭 | 💬 Project approval | Jul 2025 | First host-country authorization for Korean A6 project obtained[^krkh] |
    | India 🇮🇳 | 🤝 Bilateral agreement | Apr 2026 | Signed during Korean presidential visit; ITMO cooperation framework[^krin] |
    | Vietnam 🇻🇳 | 💬 Interest expressed | — | ASEAN carbon market linkage discussions |
    | Thailand 🇹🇭 | 💬 Interest expressed | — | — |
    | Others | — | — | — |

    > Korea's Article 6 engagement is accelerating: the Indonesia MOU (2024) and Cambodia project approval (2025) show increasing ASEAN focus. The India agreement (April 2026) marks Korea's first formal acquiring-country bilateral outside ASEAN. Primary source verification: [Korea MOE](https://www.me.go.kr/); [Indonesia MOU — Fairatmos analysis](https://www.fairatmos.com/blog/indonesia-and-south-korea-join-forces-under-article-6-a-green-leap-forward-in-carbon-emission-reduction); [India-Korea — Carbon Credits](https://carboncredits.com/india-and-south-korea-sign-article-6-2-deal-as-global-carbon-trading-gains-momentum/).

    [^krid]: Korea-Indonesia Article 6 MOU signed June 2024 by Trade Minister Inkyo Cheong and Indonesian Coordinating Minister Airlangga Hartarto; source: Fairatmos analysis: <https://www.fairatmos.com/blog/indonesia-and-south-korea-join-forces-under-article-6-a-green-leap-forward-in-carbon-emission-reduction>. Government primary source: Korea MOTIE and Indonesian CMEA.
    [^krkh]: Cambodia granted first host-country Article 6 project approval to Korea in July 2025; source: reccessary.com analysis: <https://www.reccessary.com/en/insight/asia-pushes-ahead-on-article-6>. Government confirmation pending via Cambodia MoE.
    [^krin]: India-Korea Article 6.2 bilateral agreement signed April 2026 during Korean presidential visit to India; source: Carbon Credits: <https://carboncredits.com/india-and-south-korea-sign-article-6-2-deal-as-global-carbon-trading-gains-momentum/>. Government confirmation: Korea Ministry of Foreign Affairs.

    ## Methodology Approach

    | Dimension | Korea Approach |
    |-----------|---------------|
    | Registry type | TBD — K-ETS Phase 4 rules under design |
    | Expected approach | Likely to follow existing international standards (Verra VCS, Gold Standard) or A6.2 bilateral agreements with FOEN/Energimyndigheten-style review |
    | Current KCU (Korean Credit Unit) | CDM-origin credits partially accepted under Phase 3 |
    | A6.2 compatibility | K-ETS Phase 4 design aims to align KCU with A6.2 ITMOs — alignment criteria not finalized [^krpha4] |

    ## Carbon Pricing System

    Korea's **K-ETS** is the largest carbon trading system in Asia (outside China's national ETS). Operating since 2015, it covers ~685 companies representing ~73% of national GHG emissions.

    | Phase | Period | Price Range | International Credit Rules |
    |-------|--------|-------------|--------------------------|
    | Phase 1 | 2015–2017 | KRW 8,000–11,000/t | None permitted |
    | Phase 2 | 2018–2020 | KRW 10,000–25,000/t | CDM credits ≤10% |
    | Phase 3 | 2021–2025 | KRW 8,000–18,000/t | External credits ≤5% (mostly CDM) |
    | Phase 4 | 2026–2030 | Est. KRW 20,000–40,000/t | A6.2-compatible credits expected [^krpha4] |

    Korea's domestic carbon price is well below Switzerland or Sweden (US$6–13 vs. US$105–133), limiting procurement price capacity. However, K-ETS Phase 4 price escalation is expected as free allocation decreases, which will make international A6.2 credits more economically attractive for Korean covered entities.

    ## Procurement Track Record & Demand

    | Metric | Data |
    |--------|------|
    | Active A6.2 bilateral agreements | **1 formal** (India, Apr 2026) + MOU (Indonesia) + project-level (Cambodia) |
    | **K-ETS Phase 4 A6 eligibility criteria** | **Confirmed**: ex-post credits only; post-2010 vintage; A6.2 authorization required[^krpha4criteria] |
    | **K-MSR (Market Stability Reserve)** | 🔄 **Pending** — K-ETS Phase 4 MSR design under finalisation[^krmsr] |
    | KCU usage (Phase 3) | Limited; low price has reduced incentive |
    | Phase 4 demand outlook | Significant — 685 covered firms face increasing obligations; 5% external credit cap[^krpha4] |
    | ASEAN cooperation | ACMC (ASEAN Carbon Market Cooperation) context; Korea is an observer |

    **Demand outlook**: Korea's potential demand for A6.2 credits is large in absolute terms (covered emissions ~600 Mt CO₂e/yr) but remains contingent on Phase 4 design decisions. If KCU eligibility is extended to A6.2 ITMOs with a 5%+ cap and price reaches KRW 30,000–40,000/t (≈ US$22–29), Korea would become a significant buyer — particularly from ASEAN JCM countries. The 2025–2026 period is critical for policy finalization.

    [^krndc]: Korea NDC: <https://unfccc.int/NDCREG>
    [^krpha4]: K-ETS Phase 4 design discussion: Ministry of Environment Korea; ICAP Korea ETS profile: <https://icapcarbonaction.com/en/ets/korea-emissions-trading-scheme>
    [^krpha4criteria]: K-ETS Phase 4 A6.2 credit eligibility criteria confirmed: (1) **ex-post credits only** (no ex-ante / forward credits); (2) **post-2010 vintage** minimum; (3) A6.2 host country authorization required. Source: Korea MOE; Carbon Pulse (2025–2026)
    [^krmsr]: K-ETS Market Stability Reserve (K-MSR) — Phase 4 design element pending finalization; intended to manage allowance price volatility: Korea MOE; ICAP

    ## Key Sources
    - Korea NDC: <https://unfccc.int/NDCREG>
    - ICAP K-ETS: <https://icapcarbonaction.com/en/ets/korea-emissions-trading-scheme>
    - ACMC: <https://asean.org/asean-carbon-market-cooperation/>

=== "🇯🇵 日本語"

    ## サマリー
    韓国はアジア最大規模の排出権取引制度（K-ETS、2015年〜）を運営するA6.2調達国候補。2030年NDC（2018年比40%削減）達成に向け、K-ETSフェーズ4（2026〜2030）でA6.2適合クレジットの受入拡大を設計中。**インドネシアとの二国間MOU（2024年6月）**、カンボジアでの案件レベル授権取得（2025年7月）、インドとのA6.2二国間協定締結（2026年4月）と、調達活動が加速している。[^krndc]

    ## 基本情報
    | 項目 | 内容 |
    |------|------|
    | NDC目標 | 2030年に2018年比40%削減、ネットゼロ2050 |
    | 炭素価格制度 | K-ETS（韓国排出権取引制度、2015年〜） |
    | 炭素価格（2024〜2025） | KRW 8,000〜18,000/t（≈US$6〜13） |
    | 炭素価格（2030年予測） | KRW 20,000〜40,000/t（≈US$15〜29） |
    | 国際クレジット上限 | 排出枠の5%（K-ETSフェーズ3） |
    | 主管省庁 | 環境部（MOE）、外交部 |
    | A6.2の役割 | 調達国候補（枠組み検討中） |

    ## NDC達成見込み

    | 指標 | 評価 |
    |------|------|
    | 現在の進捗 | 🟡 概ね順調だが鉄鋼・石化など重工業が難所 |
    | 2030年目標（40%） | 🟡 フェーズ4の価格引き上げ＋国際クレジット活用が必要 |
    | 国際クレジット活用 | K-ETSフェーズ4でA6.2適合クレジット受入経路を設計中（未確定） |
    | GST後の方向性 | 2035年NDC引き上げ見込み。A6.2経路は2026年前に確定が必要 |

    ## 二国間A6.2協定状況

    | ホスト国 | 状況 | 時期 | 備考 |
    |---------|------|------|------|
    | インドネシア 🇮🇩 | 📋 MOU | 2024年6月 | Article 6炭素プロジェクト協力MOU；産業系案件中心[^krid] |
    | カンボジア 🇰🇭 | 💬 案件承認 | 2025年7月 | 韓国向け初のA6.2案件レベル授権取得[^krkh] |
    | インド 🇮🇳 | 🤝 二国間協定 | 2026年4月 | 韓国大統領訪印時に締結；ITMO協力枠組み[^krin] |
    | ベトナム 🇻🇳 | 💬 関心表明 | — | ASEAN炭素市場連携の文脈 |
    | タイ 🇹🇭 | 💬 関心表明 | — | — |
    | その他 | — | — | — |

    > 韓国のA6.2調達活動は加速中。インドネシアMOU（2024年）・カンボジア案件承認（2025年）・インド二国間協定（2026年4月）と段階的に拡大。一次ソース確認は各国政府HP（韓国環境部・インドネシア経済調整省）を参照。

    ## 方法論アプローチ

    | 項目 | 内容 |
    |------|------|
    | 方法論種別 | 未確定（K-ETSフェーズ4設計中） |
    | 想定アプローチ | Verra VCS・GoldStandardなど既存国際標準、またはFOEN/Energimyndigheten型審査 |
    | KCU（韓国クレジット単位） | フェーズ3はCDM由来クレジットの一部を受入 |
    | A6.2適合性 | フェーズ4でKCUをA6.2 ITMOと整合させることを目指すが、基準は未確定 [^krpha4] |

    ## 炭素価格制度の概要

    **K-ETS**はアジア最大の国内キャップ＆トレード制度（中国ETS除く）。2015年開始、約685社（国家GHG排出量の約73%をカバー）。フェーズ3（2021〜2025年）の炭素価格はKRW 8,000〜18,000/t（≈US$6〜13）と低水準。フェーズ4（2026〜2030）では無償割当削減に伴い価格上昇を見込む。A6.2適合クレジットの受入が認められれば、ASEAN JCM国からの調達が経済合理的になる。

    ## 調達実績・需要見通し

    - 現時点でA6.2二国間協定はゼロ
    - フェーズ3のKCU使用は限定的（低価格のため調達インセンティブ薄）
    - フェーズ4の設計次第で大きな調達需要が発生する潜在力あり
    - カバード排出量≈6億t/yr × 5%上限 = 年間最大3,000万t規模の潜在需要

    ## 関連ページ
    - [マトリクス](../matrix.md)
    - [ITMO](../concepts/itmo.md)
    - [Corresponding Adjustment](../concepts/corresponding_adjustment.md)
