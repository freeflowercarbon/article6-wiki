# India 🇮🇳 | インド — Article 6.2 Host & Emerging Compliance Market
...
=== "🇬🇧 English"
    ...
    ## CCTS — Carbon Credit Trading Scheme (Compliance Market)

    India's CCTS is moving from policy to live trading in 2026. It has two arms:[^inccts]

    | Arm | Description | Status |
    |-----|-------------|--------|
    | **Compliance Mechanism** | Mandatory emission-intensity targets (GEI targets) for obligated entities | ✅ In force FY2025–26 — **~490 entities, 7 sectors** |
    | **Offset Mechanism** | Voluntary registration for non-obligated entities (e.g., farming, forestry) | 🟡 Operationalizing early 2026 |

    **Sector notification sequence**:

    | Phase | Sectors | Notified |
    |-------|---------|----------|
    | First | Aluminium, cement, chlor-alkali, pulp & paper | Oct 2025 |
    | Second | Petroleum refining, petrochemicals, textiles | Jan 2026 |

    - **First compliance trades** (Carbon Credit Certificates, CCCs) expected by **October 2026**.
    - Trading platform launch flagged for 2026 (Power Ministry).

    ## Article 6.2 Position
    ...
    > **Investor note**: India's vast mitigation potential (RE, efficiency, biogas, cookstoves) makes it an attractive A6.2 host on paper, but the **interaction between the domestic CCTS and A6.2 export authorization is the key variable** — credits absorbed by domestic compliance demand are not available for ITMO export, and India has signaled it will prioritize its own NDC. See [Host-Country ITMO Caps](../concepts/host_country_caps.md).

    ## Key Sources
    - India NDC: <https://unfccc.int/NDCREG>
    - CCTS (ICAP profile): <https://icapcarbonaction.com/en/ets/indian-carbon-credit-trading-scheme>
    - JCM India (GEC): <https://gec.jp/jcm/>
...
=== "🇯🇵 日本語"
    ...
    ## CCTS — 炭素クレジット取引スキーム（コンプライアンス市場）

    インドのCCTSは2026年に政策から実取引へ移行。2つの仕組みを持つ：[^inccts]

    | 仕組み | 内容 | 状況 |
    |--------|------|------|
    | **コンプライアンス機構** | 対象事業者に排出原単位（GEI）目標を課す義務制 | ✅ FY2025–26発効 — **約490事業者・7セクター** |
    | **オフセット機構** | 非対象事業者（農業・林業等）の任意登録 | 🟡 2026年初に運用開始 |

    **セクター告示の順序**：第1次（アルミ・セメント・塩素アルカリ・製紙、2025年10月）、第2次（石油精製・石油化学・繊維、2026年1月）。**初のコンプライアンス取引（CCC）は2026年10月見込み**。

    ## Article 6.2の立場

    | パートナー | 制度 | 状況 | 備考 |
    |-----------|------|------|------|
    | 日本 🇯🇵 | JCM | ✅ MoC 2025年8月29日 | JCM第30番目；案件パイプライン形成中 |
    | 韓国 🇰🇷 | A6.2二国間 | 🤝 2026年4月24日 | 越境炭素市場協力枠組み |
    | その他 | — | ⚪ | 広範な輸出授権には慎重 |

    > **投資家メモ**：インドの膨大な削減ポテンシャル（再エネ・省エネ・バイオガス・改良かまど）は魅力的だが、**国内CCTS需要とA6.2輸出授権の相互作用が最重要変数**。国内コンプライアンス需要に吸収されるクレジットはITMO輸出に回らず、インドは自国NDCを優先する姿勢。[ホスト国の移転上限](../concepts/host_country_caps.md)参照。

    ## 関連ページ
    - [マトリクス](../matrix.md)
    - [JCM](../concepts/jcm.md)
    - [韓国](korea.md)
    - [ホスト国の移転上限](../concepts/host_country_caps.md)
