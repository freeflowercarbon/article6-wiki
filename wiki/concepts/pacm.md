# PACM — Paris Agreement Crediting Mechanism (Article 6.4)

=== "🇬🇧 English"

    ## Overview

    The **Paris Agreement Crediting Mechanism (PACM)**, established under **Article 6.4** of the Paris Agreement, is the successor to the Clean Development Mechanism (CDM). It creates a UNFCCC-supervised multilateral crediting system where emission reductions are certified and traded internationally under centralized governance — unlike Article 6.2's bilateral approach.

    **Key distinction from Article 6.2**: Under PACM, the UNFCCC A6.4 Supervisory Body certifies methodologies and credits centrally. Under Article 6.2, two governments bilaterally authorize ITMOs using their own Joint Committee. PACM provides more standardization but less flexibility; A6.2 is faster and more flexible but requires country-by-country negotiation.

    ## Governance Structure

    | Element | Detail |
    |---------|--------|
    | Legal Basis | Article 6.4, Paris Agreement |
    | Supervisory Body | Article 6.4 Supervisory Body (A6.4SB) |
    | Established | COP26, Glasgow, November 2021 |
    | Methodological framework adopted | A6.4SB 10th Meeting, October 2024 |
    | Full operationalization | COP29, Baku, November 2024 |
    | Registry | UNFCCC centralized registry — **"Article 6 Pipeline" platform (joint UNEP CCC/UNFCCC) launched March 2026**; reported "on track for year-end 2026 launch" (secondary source, not independently confirmed)[^pacmregistry] |
    | Unit Name | Article 6.4 Emission Reduction (A6.4ER) |

    ## Two Types of A6.4 Units

    | Type | Name | Use | CA Required |
    |------|------|-----|-------------|
    | Type A | Mitigation Contribution (MC) | Host country NDC accounting only | No |
    | Type B | ITMO-eligible A6.4ER | International transfer; acquiring country NDC use | Yes |

    Only Type B units (with Corresponding Adjustment) can be used by acquiring countries to meet their NDCs. Type A units contribute to the host country's NDC only.

    ## Latest Status — COP29 Outcomes (November 2024)

    COP29 in Baku was a critical breakthrough for Article 6 overall. For Article 6.4 specifically:[^cop29a64]

    | Decision | Detail |
    |----------|--------|
    | Methodological framework | Adopted — establishes standards for additionality, baselines, leakage, permanence |
    | Removal activities standard | Adopted — specific rules for carbon removal (forestry, blue carbon, DACCS) |
    | CDM transition provisions | Confirmed — CDM projects can apply by Dec 2025 (extended) for PACM transition; strict upgrade criteria |
    | Registry design | Approved in principle — centralized UNFCCC registry architecture confirmed |
    | First methodology submissions | Multiple submissions under review by A6.4SB as of Q1 2026 |
    | No-objection procedure | Government letters of authorization required for project registration |

    > **Note**: Article 6.2 rules (bilateral ITMO trading) were separately finalized at COP29 — giving both tracks a complete rulebook for the first time since Paris 2015.[^cop29art6]

    ## COP30 Outcomes (Belém, November 2025)

    COP30 was an implementation COP for Article 6 — no major reopening of rules. Key results:[^cop30art6]

    | Decision | Detail |
    |----------|--------|
    | A6.4 standards | Baselines and additionality standards adopted; PACM further operationalized |
    | CDM → A6.4 funding | CDM Trust Fund to transfer **US$26.8M to A6.4** (+ up to US$5M for developing-country capacity building) |
    | **CDM shutdown** | Parties agreed the **CDM will cease operations by end-2026** (≈2.5bn credits issued since 2001) — sharpening the ~1bn-credit transition question |
    | A6.2 reporting | Further technical guidance on initial reports + structured review/feedback for cooperative approaches |
    | **Next negotiations** | ⚠️ **No formal Article 6 negotiations are scheduled until 2028** — the rulebook is now considered settled; focus shifts to implementation |

    > For Article 6.2 practitioners: the "no negotiations until 2028" status means the operating rules are stable through the current procurement wave — predictability for bilateral deals, but also no near-term fixes to known gaps (e.g., revocation/reversal handling).

    [^cop30art6]: COP30 (Belém, Nov 2025) Article 6 outcomes: A6.4 baselines/additionality adopted; CDM Trust Fund → A6.4 (US$26.8M + up to US$5M capacity); CDM to close end-2026; no formal Article 6 negotiations until 2028: Morgan Lewis <https://www.morganlewis.com/pubs/2025/12/cop30-key-outcomes-for-carbon-markets>; Carbon Herald <https://carbonherald.com/cop30-article-6-wraps-up-with-progress-in-hand-and-challenges-ahead/>; Smith School (Oxford) <https://www.smithschool.ox.ac.uk/news/article-6-focus-outcomes-cop30>. SB64 (Bonn, Jun 2026) made little tangible progress ("gridlock"); A6.2 discussion focused on infrastructure operation/maintenance funding: Carbon Brief <https://www.carbonbrief.org/bonn-climate-talks-key-outcomes-from-the-june-2026-un-climate-conference/>

    ## Key Milestones (Full Timeline)

    | Date | Milestone |
    |------|-----------|
    | Nov 2021 (COP26) | A6.4 Supervisory Body established; initial work plan adopted |
    | 2022 | A6.4SB meetings #1–4; methodology standards in draft |
    | 2023 | A6.4SB meetings #5–8; additionality framework debated; CDM transition rules disputed |
    | Oct 2024 | A6.4SB 10th meeting — methodological framework and removal standards adopted |
    | Nov 2024 (COP29) | Full operationalization — complete rules package adopted by CMA |
    | Dec 2024 | CDM transition application window open |
    | 2025 | First PACM project registrations expected; A6.4SB begins methodology reviews |
    | **Feb 26, 2026** | **HISTORIC: First-ever A6.4ERs issued** — Myanmar efficient cookstoves PoA 10471: **58,428 tCO₂e** issued (project shows ~603,368 t issuable; South Korea acquiring)[^firstpacm] |
    | **Mar 2026** | **2nd issuance approval: PoA 10415** (joining Myanmar PoA 10471) — PACM issuance pipeline widening[^pacm2nd] |
    | **2026** | 6-month deadline extension keeps PACM open to **~1 billion CDM transition credits**; integrity controversy over Myanmar junta links (Carbon Market Watch + Global Forest Coalition call for suspension/investigation)[^pacmcontroversy] |
    | **Jun 30, 2026** | **Host-country approval deadline passed — pipeline sharply narrowed**: only **415 of 1,512 CDM project activities (27%)** secured host-country approval to transition, representing up to **617M tCO₂e** (well below the earlier ~1bn framing). **China blocked 527 projects (~251M t)**; **India approved 0 of 460 projects (~182M t)**. New deadlines: **PDD documentation due Dec 31, 2026** (Brazil leads the backlog with 92 pending); non-transitioned CDM credits face **administrative cancellation Jul 1, 2027**[^pacmjun2026] |
    | **Jul 10, 2026** | **65% of countries (129 of 198) have appointed Designated National Authorities for PACM**; the joint UNEP Copenhagen Climate Centre / UNFCCC Secretariat **"Article 6 Pipeline" platform launched March 2026**[^pacmregistry] |
    | **Jul 27–30, 2026** | **A6.4 Supervisory Body 22nd meeting, Bonn — concluded**: approved a **grid-connected renewable electricity methodology** (3rd A6.4-native methodology, after landfill gas and N2O abatement); **deferred** a household cooking-energy-efficiency (cookstove) methodology decision pending further work on non-permanence, uncertainty and conservativeness; also updated registry procedures (issuing/holding/transferring/tracking credits）（要確認 — corroborated by a secondary source only; primary UNFCCC decision document not yet located）[^a64sb22] |
    | 2027+ | Scale-up phase; CORSIA eligibility decision expected; next A6.4SB meeting (SBM 23) Oct 5–9, 2026, Bonn |

    [^firstpacm]: First Article 6.4 credits (A6.4ERs) issued February 26, 2026 — Myanmar efficient cookstoves PoA 10471, 58,428 tCO₂e issued (≈603,368 t issuable; 648,783 t reductions for vintage Jan 2021–May 2022); acquiring country: South Korea: UNFCCC A6.4SB registry; S&P Global <https://www.spglobal.com/energy/en/news-research/latest-news/energy-transition/022626-un-greenlights-first-credits-under-article-64-from-myanmar-cookstove-project>
    [^pacm2nd]: Second PACM issuance approval: PoA 10415 (March 2026), following Myanmar PoA 10471: Fastmarkets <https://www.fastmarkets.com/insights/first-issuances-approved-under-pacm-from-myanmar-cookstove-project-with-loa/>
    [^pacmcontroversy]: PACM granted a 6-month deadline extension keeping the door open to ~1bn CDM transition credits; the Myanmar cookstove project faces an integrity controversy (junta links; remote verification) with Carbon Market Watch and Global Forest Coalition calling for suspension and independent investigation: Fastmarkets <https://www.fastmarkets.com/insights/article-6-4-a-six-month-deadline-extension-keeps-pacms-doors-open-to-roughly-1bn-cdm-credits/>; Carbon Market Watch <https://carbonmarketwatch.org/2026/06/09/>; Global Forest Coalition <https://globalforestcoalition.org/first-un-paris-agreement-high-integrity-carbon-credits-linked-to-myanmar-junta-new-report-finds/>
    [^pacmjun2026]: Host-country approval deadline (Jun 30, 2026): only 415 of 1,512 CDM project activities (27%, up to 617M tCO₂e) secured approval to transition to PACM; China blocked 527 projects (~251M t), India approved 0 of 460 (~182M t); new PDD documentation deadline Dec 31, 2026 (Brazil leads backlog, 92 pending); non-transitioned CDM credits face administrative cancellation Jul 1, 2027: Fastmarkets <https://www.fastmarkets.com/insights/host-country-approvals-narrow-pacm-pipeline-as-backlog-and-methodology-rollout-weigh-on-supply/> (paywalled beyond summary); corroborated by H2 Bulletin (Jul 10, 2026) citing UNEP Copenhagen Climate Centre "Article 6 Pipeline" data <https://h2bulletin.com/two-thirds-of-countries-now-have-article-6-crediting-authorities-un-data-shows/>
    [^pacmregistry]: 129 of 198 countries (65%) had appointed Designated National Authorities for PACM as of Jul 10, 2026; joint UNEP Copenhagen Climate Centre/UNFCCC Secretariat "Article 6 Pipeline" platform launched March 2026: H2 Bulletin <https://h2bulletin.com/two-thirds-of-countries-now-have-article-6-crediting-authorities-un-data-shows/>; UNFCCC international registry "on track for year-end 2026 launch" per Carbon Pulse <https://carbon-pulse.com/529597/> (headline-level, paywalled, secondary — not independently confirmed against a primary UNFCCC document as of this crawl)
    [^a64sb22]: A6.4 Supervisory Body 22nd meeting, Bonn, Jul 27–30, 2026 — concluded Jul 30 with approval of a grid-connected renewable electricity methodology (3rd A6.4-native methodology) and deferral of a cookstove methodology decision: Carbon Herald <https://carbonherald.com/renewable-energy-methodology-cleared-in-major-expansion-of-un-carbon-market/>; White & Case alert "Progress made on Paris Agreement Article 6.4 at Bonn; Talks to Continue in Dubai" (headline corroboration only, article inaccessible/403); UN Indico event page <https://indico.un.org/event/1024170/> (logistics only). No primary UNFCCC decision document located as of this crawl (2026-08-02) — treat methodology-approval detail as medium confidence pending primary confirmation

    ## A6.4SB — Active Methodology Reviews (updated Jul 2026)

    | Sector | Status | Notes |
    |--------|--------|-------|
    | **Grid-connected renewable electricity** | 🟢 **Approved (Jul 30, 2026)** | Third A6.4-native methodology approved at SB22, Bonn（要確認、一次資料未確認）[^a64sb22] |
    | Efficient cookstoves | 🟡 Under review | AMS-II.G adaptation; **methodology decision deferred at SB22 (Jul 2026)** pending further work on non-permanence, uncertainty and conservativeness[^a64sb22] |
    | Avoided deforestation (REDD+) | 🔴 Not yet approved | Complex permanence/leakage rules |
    | Carbon removal (reforestation) | 🟡 Under review | New removal activity standards apply |
    | **Landfill gas flaring/utilization** | 🟢 **Approved (Oct 2025)** | First A6.4-native methodology approved[^pacmmethods] |
    | **N2O abatement (nitric acid production)** | 🟢 **Approved (May 2026)** | Second A6.4-native methodology approved[^pacmmethods] |

    > Only **64 of the 415** approved-transition projects are using Article 6.4-native methodologies as of Jul 2026 — the remaining **351 remain on legacy CDM methodologies** pending further A6.4SB approvals.[^pacmmethods]
    > A6.4SB methodology registry: <https://unfccc.int/sb-a64>

    ## Comparison: PACM vs. CDM vs. Article 6.2

    | Criterion | CDM (legacy) | PACM (A6.4) | Article 6.2 |
    |-----------|-------------|-------------|-------------|
    | Governance | UNFCCC Executive Board | UNFCCC A6.4SB | Bilateral governments |
    | Methodology approval | UNFCCC EB | A6.4SB | Bilateral Joint Committee |
    | Additionality standard | Conservative (project-level) | Enhanced (updated baselines) | Bilateral agreement |
    | CA requirement | No | Yes (Type B only) | Yes |
    | Registry | CDM registry (winding down) | UNFCCC central registry | Host country + bilateral |
    | Speed to registration | 18–36 months | Uncertain (target <18 months) | Faster (bilateral) |
    | Status (Jul 2026) | Wind-down; only 415/1,512 activities (27%) approved to transition as of Jun 30, 2026 deadline | **First credits issued Feb 26, 2026** (Myanmar, 58,428 t); 2 native methodologies approved | Active; ITMOs issued |
    | NBS / REDD+ | Widely used (but no CA) | Under methodology review | Used bilaterally (JCM, SG IA) |

    ## CDM to PACM Transition

    - CDM projects may apply to transition to PACM by the specified deadline (extended to Dec 2025 for application; **host-country approval deadline was Jun 30, 2026**)
    - Transition requirements: updated baseline (typically 10-year refresh), enhanced additionality, host country re-authorization, A6.4SB approval
    - **Most legacy CDM projects face barriers**: newer additionality standards disqualify older fossil-fuel efficiency projects; many large REDD+ projects cannot easily adapt to PACM's permanence requirements
    - CDM credits already issued: some are CORSIA-eligible (vintage 2016+) but do NOT automatically transfer to PACM — they remain as CDM credits
    - **Actual result (Jun 30, 2026 deadline)**: only **415 of 1,512 CDM activities (27%)** secured host-country approval — far below earlier ~10–20%-of-active-projects estimates in absolute terms once host-country blocking is factored in. **China blocked 527 projects; India approved 0 of 460.** Remaining PDD documentation is due **Dec 31, 2026**; non-transitioned CDM credits face administrative cancellation **Jul 1, 2027**[^pacmjun2026]

    ## PACM and CORSIA Interface

    | Question | Status (Q1 2026) |
    |----------|-----------------|
    | Are A6.4ERs CORSIA-eligible? | 🔴 **Not yet decided** — ICAO and UNFCCC in formal dialogue |
    | Expected timeline | ICAO CORSIA Technical Advisory Body review expected 2026–2027 |
    | Implication if approved | A6.4ER would become primary A6 pathway into aviation market |
    | Implication if denied | A6.4 and CORSIA remain separate tracks; voluntary markets remain bridge |

    > CORSIA details: [CORSIA](../buyers/corsia.md)

    ## PACM vs. Article 6.2 — Which Track for NBS?

    For Nature-Based Solutions (forest, peatland, blue carbon):

    | Consideration | PACM (A6.4) | Article 6.2 |
    |--------------|-------------|-------------|
    | REDD+ methodology approved? | 🔴 Not yet | 🟡 Bilateral JC approval (JCM) / Verra VCS (SG, CH) |
    | Permanence buffer requirement | Strict (buffer pool mandatory) | Bilateral negotiation |
    | Speed | Slower (A6.4SB bottleneck) | Faster (bilateral) |
    | Market demand | Uncertain (no CORSIA link yet) | Confirmed demand (Switzerland, Singapore, Japan) |
    | Recommended track (2025–2027) | 🟡 Wait for methodology approval | ✅ Active track for immediate action |

    **Practical implication**: For project developers seeking Article 6 credits in 2025–2027, Article 6.2 bilateral channels (JCM, Singapore IA, Switzerland FOEN) remain more actionable than PACM. PACM will become more important post-2027 once methodologies are approved and the registry is operational.

    [^cop29a64]: UNFCCC COP29 Article 6 outcomes: <https://unfccc.int/cop29>; Carbon Pulse COP29 A6 summary: <https://carbon-pulse.com>
    [^cop29art6]: Article 6.2 rules also finalized at COP29 — first complete rulebook since Paris 2015: <https://unfccc.int/process/conferences/cop29>
    [^cdmtrans]: CDM transition estimate based on OECD/IGES analysis; actual numbers pending A6.4SB determinations — see [^pacmjun2026] for the actual Jun 30, 2026 deadline outcome
    [^pacmmethods]: A6.4-native methodologies approved: landfill gas flaring/utilization (Oct 2025), N2O abatement in nitric acid production (May 2026); only 64 of the 415 approved-transition projects use A6.4-native methodologies as of Jul 2026, the remaining 351 still on legacy CDM methodologies: Fastmarkets <https://www.fastmarkets.com/insights/host-country-approvals-narrow-pacm-pipeline-as-backlog-and-methodology-rollout-weigh-on-supply/>; H2 Bulletin <https://h2bulletin.com/two-thirds-of-countries-now-have-article-6-crediting-authorities-un-data-shows/>

    ## Key Sources
    - UNFCCC Article 6.4 Supervisory Body: <https://unfccc.int/sb-a64>
    - A6.4SB meeting reports: <https://unfccc.int/process/bodies/constituted-bodies/a6-4-supervisory-body>
    - COP29 Article 6 outcomes: <https://unfccc.int/cop29>
    - Carbon Pulse A6 tracker: <https://carbon-pulse.com>

=== "🇯🇵 日本語"

    ## 概要

    **PACM（パリ協定クレジット制度）**は、パリ協定**Article 6.4**に基づくCDM（クリーン開発メカニズム）の後継制度。UNFCCC A6.4監督機関（A6.4SB）が方法論とクレジットを中央集権的に認証する多国間の仕組みで、二国間合意で運営されるArticle 6.2とは対照的。

    **Article 6.2との違い**: PACMはUNFCCCの監督機関が一元認証。Article 6.2は二国間政府が各自のJoint CommitteeでITMOを授権。PACMの方が標準化されているが、柔軟性は低い。Article 6.2は二国間交渉なので迅速だが、国ごとの交渉が必要。

    ## ガバナンス構造

    | 項目 | 内容 |
    |------|------|
    | 法的根拠 | パリ協定 Article 6.4 |
    | 監督機関 | Article 6.4監督機関（A6.4SB） |
    | 設立 | COP26（グラスゴー、2021年11月） |
    | 方法論フレームワーク採択 | A6.4SB第10回会合（2024年10月） |
    | 本格運用化 | COP29（バクー、2024年11月） |
    | 登録簿 | UNFCCC中央登録簿 — **「Article 6 Pipeline」プラットフォーム（UNEP CCC/UNFCCC共同）2026年3月稼働開始**；2026年末稼働見込みとの報道あり（二次情報源、未確認）[^pacmregistry] |
    | ユニット名称 | Article 6.4排出削減量（A6.4ER） |

    ## 2種類のA6.4ユニット

    | 種類 | 名称 | 用途 | CA必要 |
    |------|------|------|--------|
    | Type A | 緩和貢献クレジット（MC） | ホスト国NDCのみ | 不要 |
    | Type B | ITMO適格A6.4ER | 国際移転・調達国NDC充当 | 必要 |

    国際移転・調達国のNDC充当に使えるのはType B（CA付き）のみ。

    ## 最新状況 — COP29成果（2024年11月）

    バクーCOP29はArticle 6全体の重要な転換点。Article 6.4について：[^cop29a64]

    | 決定事項 | 内容 |
    |---------|------|
    | 方法論フレームワーク | 採択済み — 追加性・ベースライン・漏洩・恒久性の基準を確立 |
    | 除去活動基準 | 採択済み — 植林・ブルーカーボン・DACCS等の除去活動専用ルール |
    | CDM移行規定 | 確定 — CDMプロジェクトは2025年12月までにPACM移行申請可能 |
    | 登録簿設計 | 基本合意 — UNFCCC中央登録簿アーキテクチャが確定 |
    | 方法論審査 | 複数の申請がA6.4SBで審査中（2026年Q1時点） |

    > Article 6.2ルール（二国間ITMO取引）もCOP29で別途確定。パリ2015年以来初の完全ルールブックが完成。[^cop29art6]

    ## COP30成果（ベレン、2025年11月）

    COP30はArticle 6の「実装COP」でルールの再交渉はなし。主な結果：[^cop30art6]

    | 決定事項 | 内容 |
    |---------|------|
    | A6.4基準 | ベースライン・追加性基準を採択；PACMをさらに運用化 |
    | CDM→A6.4資金 | CDM信託基金から**A6.4へUS$2,680万**移転（＋途上国能力構築に最大US$500万） |
    | **CDM閉鎖** | **CDMは2026年末までに運用停止**で合意（2001年以来約25億クレジット発行）— 約10億の移行問題が先鋭化 |
    | A6.2報告 | 協調アプローチの初期報告・構造的レビュー/フィードバックの技術ガイダンス追加 |
    | **次回交渉** | ⚠️ **Article 6の正式交渉は2028年まで予定なし** — ルールブックは確定、焦点は実装へ |

    > A6.2実務者にとって「2028年まで交渉なし」は、現在の調達局面を通じて運用ルールが安定することを意味する（二国間案件の予見可能性は高まるが、既知の課題＝取消・リバーサル処理等の早期是正もない）。

    ## 主要マイルストーン（全体時系列）

    | 時期 | 内容 |
    |------|------|
    | 2021年11月（COP26） | A6.4SB設立・初期作業計画採択 |
    | 2022年 | SB第1〜4回会合、方法論基準草案 |
    | 2023年 | SB第5〜8回会合、追加性フレームワーク議論 |
    | 2024年10月 | SB第10回 — 方法論フレームワーク・除去活動基準採択 |
    | 2024年11月（COP29） | 完全運用化 — CMAで完全ルールパッケージ採択 |
    | 2024年12月 | CDM移行申請ウィンドウ開始 |
    | 2025年 | 初のPACMプロジェクト登録が見込まれる |
    | **2026年2月26日** | **【歴史的】初のA6.4ERが発行** — ミャンマー省エネかまどPoA 10471、58,428 tCO₂e発行（韓国が調達）[^firstpacm] |
    | **2026年3月** | **2件目の発行承認：PoA 10415**（ミャンマーPoA 10471に続く）[^pacm2nd] |
    | **2026年** | 6ヶ月の期限延長で**約10億のCDM移行クレジット**にPACMの門戸が開く；ミャンマー軍政との関連で完全性に懸念（Carbon Market Watch等が停止・調査を要求）[^pacmcontroversy] |
    | **2026年6月30日** | **ホスト国承認期限が到来 — パイプラインが大幅に縮小**：CDM1,512件中**415件（27%）**のみが移行承認を取得、最大**6.17億tCO₂e**相当（従来の約10億t想定を大きく下回る）。**中国は527件（約2.51億t）を承認拒否**、**インドは460件中0件を承認**。新PDD提出期限は**2026年12月31日**（ブラジルが92件で最大の未提出案件を抱える）；未移行のCDMクレジットは**2027年7月1日に失効**[^pacmjun2026] |
    | **2026年7月10日** | **198カ国中129カ国（65%）がPACM向け国家指定機関（DNA）を任命済み**；UNEP Copenhagen Climate Centre/UNFCCC事務局共同の**「Article 6 Pipeline」プラットフォームが2026年3月に稼働開始**[^pacmregistry] |
    | **2026年7月27〜30日** | **A6.4監督機関第22回会合、ボン — 終了**：**系統連系型再エネ発電の方法論を承認**（埋立地ガス・N2O削減に次ぐ3件目のA6.4固有方法論）；**かまど（cookstove）方法論の決定は非永続性・不確実性・保守性に関する追加検討のため見送り**；登録簿手続（発行・保有・移転・トラッキング）も更新（要確認 — 二次情報源による裏付けのみ、一次UNFCCC決定文書は未確認）[^a64sb22] |
    | 2027年以降 | スケールアップ、CORSIA適格性決定見込み；次回A6.4SB会合（SBM23）2026年10月5〜9日、ボン |

    ## CDMからPACMへの移行

    - CDMプロジェクトは期限までにPACM移行申請が可能（申請自体は2025年12月まで延長；**ホスト国承認期限は2026年6月30日**）
    - 移行条件：ベースライン更新（通常10年更新）、追加性の強化、ホスト国再授権、A6.4SB承認
    - **ほとんどのCDMプロジェクトは移行困難**：古い化石燃料効率化プロジェクトは新追加性基準をクリアできず。REDD+は恒久性要件への対応が難しい
    - **実際の結果（2026年6月30日期限）**：CDM1,512件中**415件（27%）のみ**が移行承認を取得 — ホスト国による承認拒否を織り込むと従来の10〜20%推定を絶対数では下回る。**中国は527件を拒否、インドは460件中0件を承認**。残るPDD提出期限は**2026年12月31日**、未移行CDMクレジットは**2027年7月1日に失効**[^pacmjun2026]

    ## A6.4SB — 方法論審査の最新状況（2026年7月更新）

    | 分野 | 状況 | 備考 |
    |------|------|------|
    | **系統連系型再エネ発電** | 🟢 **承認済み（2026年7月30日）** | SB22（ボン）でA6.4固有方法論として3件目を承認（要確認、一次資料未確認）[^a64sb22] |
    | 効率的かまど | 🟡 審査中 | AMS-II.G応用；**SB22（2026年7月）で方法論決定を見送り** — 非永続性・不確実性・保守性の追加検討待ち[^a64sb22] |
    | 森林減少回避（REDD+） | 🔴 未承認 | 恒久性・漏洩ルールが複雑 |
    | 炭素除去（植林） | 🟡 審査中 | 新除去活動基準を適用 |
    | **埋立地ガスフレアリング/利用** | 🟢 **承認済み（2025年10月）** | A6.4固有方法論として初承認[^pacmmethods] |
    | **硝酸製造でのN2O削減** | 🟢 **承認済み（2026年5月）** | A6.4固有方法論として2件目[^pacmmethods] |

    > 移行承認済み415件のうち、A6.4固有方法論を使用しているのは**わずか64件**（2026年7月時点）。残る**351件は旧CDM方法論のまま**、A6.4SBの追加承認待ち。[^pacmmethods]

    ## PACMとCORSIAの連携

    | 論点 | 2026年Q1時点の状況 |
    |------|------------------|
    | A6.4ERはCORSIA適格？ | 🔴 **未決定** — ICAOとUNFCCCが正式対話中 |
    | 見込み時期 | ICAOのCORSIA技術諮問委員会が2026〜2027年に審査予定 |
    | 承認された場合 | A6.4ERが航空市場向けの主要Article 6ルートに |
    | 否認された場合 | A6.4とCORSIAは別トラックのまま |

    ## NBS（自然に基づく解決策）ではどちらを選ぶか

    | 考慮点 | PACM（A6.4） | Article 6.2 |
    |--------|-------------|-------------|
    | REDD+方法論の承認 | 🔴 未承認 | 🟡 JCM：JC承認済み / SG・CH：Verra VCS活用 |
    | 恒久性バッファー | 厳格（バッファープール必須） | 二国間交渉による |
    | 登録スピード | 遅い（A6.4SBがボトルネック） | 速い（二国間） |
    | 市場需要 | 未確定（CORSIAリンクなし） | 確定（スイス・シンガポール・日本） |
    | 2025〜2027の推奨 | 🟡 方法論承認待ち | ✅ 即戦力 |

    **実務的結論**: 2025〜2027年においては、Article 6.2の二国間チャネル（JCM・SG IA・スイスFOEN）の方が、PACM待ちより実行可能性が高い。PACM本格活用は方法論承認・登録簿稼働後（2027年以降）が現実的。

    ## 関連ページ
    - [ITMO](itmo.md)
    - [Corresponding Adjustment](corresponding_adjustment.md)
    - [CORSIA](../buyers/corsia.md)
    - [Reversal Risk](reversal_risk.md)
    - [マトリクス](../matrix.md)
